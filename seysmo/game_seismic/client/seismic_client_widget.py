...
Принимает данные по TCP, анализирует (STA/LTA, пикинг, локация),
отображает графики, STA/LTA кривую, карту с эпицентром.
"""
...
class SeismicClientWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        for sensor in self.sensors:
            ...
            # Правильно: создаём, позиционируем, добавляем — отдельными шагами
    ...
    def _on_chunk_received(self, chunk: dict):
        ...
        buf.append('Z', np.arange(len(z)) / 100.0 + timestamp, z)
    ...
    def _redraw_all(self):
        """Перерисовывает все графики (10 Гц)."""
        now = 0.0
        for sensor in self.sensors:
